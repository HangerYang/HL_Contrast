...
import numpy as np
from augmentor import add_edge
...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight):
        ...
        edge_index1 = add_edge(edge_index, aug1)
        edge_index2 = add_edge(edge_index, aug2)
        edge_index_high, edge_weight_high = get_laplacian(edge_index1, edge_weight, normalization="sym")
        z = self.encoder(x, edge_index, edge_weight)
        z1 = self.encoder(x, edge_index_high, edge_weight_high, normalize=False)
        z2 = self.encoder(x, edge_index2, edge_weight)
        return z, z1, z2
...
def main():
    ...
    with open('./results/nc_FBGCN_Aug_{}_{}_{}.csv'.format(args.dataset, "EA", args.aug_side), 'a+') as file:
        file.write('\n')
        ...
        file.write("high augmentation ratio = {}\n".format(args.aug))
        file.write("low augmentation ratio = {}\n".format(args.aug2))
        file.write('(E): FBGCN_AUG Mean Accuracy: {}, with Std: {}'.format(np.mean(total_result), np.std(total_result)))
        file.write('\n')
