...
from model.downstream import FBGCN_Head, GCN, GAT
...
def main():
    ...
    with open('./results/nc_{}_{}_{}.csv'.format(args.dataset,args.gnn, args.loss_type), 'a+') as file:
        ...
        for r in range(5):
            if args.preepochs != 0:
                fbconv = Pre_Train(2, data.num_features, args.hidden_dim, args.second_hidden_dim, 0.5)
                ...
                encoder_model = Encoder(pretrain_model=fbconv, hidden_dim = args.second_hidden_dim, proj_dim= args.second_hidden_dim, augmentor=(aug1, aug2)).to(device)
            ...
            if args.gnn == "gcn":
                ...
                model = FBGCN_Head(2,data.num_features, args.hidden_dim, args.second_hidden_dim, data.num_classes, 0.5).to(device)
            model.train()
            if (args.preepochs != 0):
                od = OrderedDict()
