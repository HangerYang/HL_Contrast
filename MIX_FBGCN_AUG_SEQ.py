...
class GConv(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight=None, normalize=True, diff = False):
        z = x
        if not diff:
            ...
            z = self.layers[-1](z, edge_index, edge_weight, normalize)
            z = self.activation(z)
            return z
...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight):
        ...
        edge_index_high, edge_weight_high = get_laplacian(edge_index1, edge_weight1, normalization="sym")
        z = self.encoder(x, edge_index, edge_weight)
        z_1 = self.encoder(x1, edge_index1, edge_weight1)
        z1 = self.encoder(z_1, edge_index_high, edge_weight_high, normalize = False, diff = True)
...
def main():
    ...
    with open('./results/nc_FBGCN_Aug_{}_{}.csv'.format(args.dataset, args.loss_type), 'a+') as file:
        ...
        file.write(
            f'(E): FBGCN_Aug_SEQ: Best test accuracy={test_result["accuracy"]:.4f}')
        file.write('\n')
